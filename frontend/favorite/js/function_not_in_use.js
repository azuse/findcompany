...
function colorOnly(companies){
    if (filterOptionTopCompanies['color'] == 0)
        return companies
    else{
        var ret = new Array;
        for (i in companies) {
            if (favoriteColor[companies[i].id] == filterOptionTopCompanies['color'])
                ret.push(companies[i])
        }
        return ret
    }
...
function setOptionTopCompanies() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            if (filterOptionTopCompanies['sort'] == "favoriteDESC")
                res = favoriteOrderByTimeDESC(res)
            else if (filterOptionTopCompanies['sort'] == "favoriteASC")
...
function loadMore() {
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            oldCompanies = topCompany.companies
            for (i in newCompanies) {
                oldCompanies.push(newCompanies[i])
            }
            allCompanies = oldCompanies

            if (filterOptionTopCompanies['sort'] == "favoriteDESC")
                allCompanies = favoriteOrderByTimeDESC(allCompanies)
            else if (filterOptionTopCompanies['sort'] == "favoriteASC")
                allCompanies = favoriteOrderByTimeASC(allCompanies)
            
            allCompanies = colorOnly(allCompanies)

            topCompany.companies = allCompanies
            topCompany.favoriteTime = favoriteTime
            topCompany.$nextTick(function () {
                $('.dropdown-button-custom2').dropdown({
                    inDuration: 300,
