...
//     var result = favoriteCompany.companies
...
var favoriteCompany = new Vue({
    el: '#favoriteCompanyContainer',
    data: {
        companies: [],
    ...
    methods: {
        switchFavorite: function (id, favorite, key, color) {
            if (color == favorite) {
                ...
                saveFavorite(this.companies)
                Materialize.toast('已从收藏中删除', 2000)
...
function openExcelModal(){
    excelTableVue.companies = favoriteCompany.companies
    $("#excelModal").modal('open')
}

var favoriteCompanyFilter = new Vue({
    el : "#favoriteCompanyOptionContainer",
    data : {
        filterOptionFavoriteCompanies: filterOptionFavoriteCompanies
    },
    methods:{
        updateList()
        {
